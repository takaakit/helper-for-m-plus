...
class ConvertToUnion : IMyPluginActionDelegate {
    // ˅
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IClass
                    || CppClassConverter(selectedElement).isCppClassKind(listOf(CppClassConverter.ClassKind.UNION))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            CppClassConverter(it as IClass).convertCppClassKind(listOf(CppClassConverter.ClassKind.UNION))
            println("Converted to union :".plus(it.getFullName(".")))
        }
        // ˄
    }
