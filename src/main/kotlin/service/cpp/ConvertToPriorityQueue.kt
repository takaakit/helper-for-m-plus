...
class ConvertToPriorityQueue : IMyPluginActionDelegate {
    // ˅
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IAssociation
                    || CppAssociationConverter(selectedElement).isCppCollectionKind(listOf(CppAssociationConverter.CollectionKind.PRIORITY_QUEUE))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            CppAssociationConverter(it as IAssociation).convertCppCollectionKind(listOf(CppAssociationConverter.CollectionKind.PRIORITY_QUEUE))
            if (it.memberEnds[0].navigability == "Navigable") {
                println("Converted to priority_queue :".plus(it.memberEnds[0].getFullName(".")))
            } else {
                println("Converted to priority_queue :".plus(it.memberEnds[1].getFullName(".")))
            }
        }
        // ˄
    }
