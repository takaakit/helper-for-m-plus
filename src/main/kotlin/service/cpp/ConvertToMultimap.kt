...
class ConvertToMultimap : IMyPluginActionDelegate {
    // ˅
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IAssociation
                    || CppAssociationConverter(selectedElement).isCppCollectionKind(listOf(CppAssociationConverter.CollectionKind.MULTIMAP))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            CppAssociationConverter(it as IAssociation).convertCppCollectionKind(listOf(CppAssociationConverter.CollectionKind.MULTIMAP))
            if (it.memberEnds[0].navigability == "Navigable") {
                println("Converted to multimap :".plus(it.memberEnds[0].getFullName(".")))
            } else {
                println("Converted to multimap :".plus(it.memberEnds[1].getFullName(".")))
            }
        }
        // ˄
    }
