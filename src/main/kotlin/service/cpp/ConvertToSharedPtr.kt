...
class ConvertToSharedPtr : IMyPluginActionDelegate {
    // ˅
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IAssociation
                    || CppAssociationConverter(selectedElement).isCppPointerKind(listOf(CppAssociationConverter.PointerKind.SHARED_PTR))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            CppAssociationConverter(it as IAssociation).convertCppPointerKind(listOf(CppAssociationConverter.PointerKind.SHARED_PTR))
            if (it.memberEnds[0].navigability == "Navigable") {
                println("Converted to shared pointer :".plus(it.memberEnds[0].getFullName(".")))
            } else {
                println("Converted to shared pointer :".plus(it.memberEnds[1].getFullName(".")))
            }
        }
        // ˄
    }
