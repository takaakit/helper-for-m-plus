...
class ConvertToTreeMap : IMyPluginActionDelegate {
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IAssociation
                    || JavaAssociationConverter(selectedElement).isJavaCollectionKind(listOf(JavaAssociationConverter.CollectionKind.TREE_MAP))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            JavaAssociationConverter(it as IAssociation).convertJavaCollectionKind(listOf(JavaAssociationConverter.CollectionKind.TREE_MAP))
