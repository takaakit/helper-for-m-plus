...
class ConvertToHashSet : IMyPluginActionDelegate {
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IAssociation
                    || CsharpAssociationConverter(selectedElement).isCsharpCollectionKind(listOf(CsharpAssociationConverter.CollectionKind.HASH_SET))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            CsharpAssociationConverter(it as IAssociation).convertCsharpCollectionKind(listOf(CsharpAssociationConverter.CollectionKind.HASH_SET))
