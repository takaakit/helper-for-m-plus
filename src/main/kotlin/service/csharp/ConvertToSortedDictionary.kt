...
class ConvertToSortedDictionary : IMyPluginActionDelegate {
    // ˅
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IAssociation
                    || CsharpAssociationConverter(selectedElement).isCsharpCollectionKind(listOf(CsharpAssociationConverter.CollectionKind.SORTED_DICTIONARY))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            CsharpAssociationConverter(it as IAssociation).convertCsharpCollectionKind(listOf(CsharpAssociationConverter.CollectionKind.SORTED_DICTIONARY))
