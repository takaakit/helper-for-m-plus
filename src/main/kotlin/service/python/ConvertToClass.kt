...
class ConvertToClass : IMyPluginActionDelegate {
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IClass
                    || PythonClassConverter(selectedElement).isPythonClassKind(listOf(PythonClassConverter.ClassKind.CLASS))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            PythonClassConverter(it as IClass).convertPythonClassKind(listOf(PythonClassConverter.ClassKind.CLASS))
