...
class ConvertToMap : IMyPluginActionDelegate {
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IAssociation
                    || TypescriptAssociationConverter(selectedElement).isTypescriptCollectionKind(listOf(TypescriptAssociationConverter.CollectionKind.MAP))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            TypescriptAssociationConverter(it as IAssociation).convertTypescriptCollectionKind(listOf(TypescriptAssociationConverter.CollectionKind.MAP))
