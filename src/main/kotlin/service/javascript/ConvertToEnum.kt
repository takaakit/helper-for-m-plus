...
import main.kotlin.util.converter.JavascriptClassConverter
...
class ConvertToEnum : IMyPluginActionDelegate {
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IClass
                    || JavascriptClassConverter(selectedElement).isJavascriptClassKind(listOf(JavascriptClassConverter.ClassKind.ENUM))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            JavascriptClassConverter(it as IClass).convertJavascriptClassKind(listOf(JavascriptClassConverter.ClassKind.ENUM))
