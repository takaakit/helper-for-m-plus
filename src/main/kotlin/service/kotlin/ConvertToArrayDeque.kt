...
class ConvertToArrayDeque : IMyPluginActionDelegate {
    // ˅
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IAssociation
                || KotlinAssociationConverter(selectedElement).isKotlinCollectionKind(listOf(KotlinAssociationConverter.CollectionKind.ARRAY_DEQUE))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            KotlinAssociationConverter(it as IAssociation).convertKotlinCollectionKind(listOf(KotlinAssociationConverter.CollectionKind.ARRAY_DEQUE))
            if (it.memberEnds[0].navigability == "Navigable") {
                println("Converted to ArrayDeque :".plus(it.memberEnds[0].getFullName(".")))
            } else {
                println("Converted to ArrayDeque :".plus(it.memberEnds[1].getFullName(".")))
            }
        }
        // ˄
    }
