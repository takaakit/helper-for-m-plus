...
class ConvertToCompanionObject : IMyPluginActionDelegate {
    // ˅
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IClass
                    || KotlinClassConverter(selectedElement).isKotlinClassKind(listOf(KotlinClassConverter.ClassKind.COMPANION, KotlinClassConverter.ClassKind.OBJECT))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            KotlinClassConverter(it as IClass).convertKotlinClassKind(listOf(KotlinClassConverter.ClassKind.COMPANION, KotlinClassConverter.ClassKind.OBJECT))
            println("Converted to companion object :".plus(it.getFullName(".")))
        }
        // ˄
    }
