...
class ConvertToEnumClass : IMyPluginActionDelegate {
    // ˅
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IClass
                    || KotlinClassConverter(selectedElement).isKotlinClassKind(listOf(KotlinClassConverter.ClassKind.ENUM))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            KotlinClassConverter(it as IClass).convertKotlinClassKind(listOf(KotlinClassConverter.ClassKind.ENUM))
            println("Converted to enum class :".plus(it.getFullName(".")))
        }
        // ˄
    }
