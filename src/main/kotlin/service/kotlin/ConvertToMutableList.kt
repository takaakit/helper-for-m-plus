...
class ConvertToMutableList : IMyPluginActionDelegate {
    // ˅
    ...
    override fun excludeSetElements(selectedElements: List<INamedElement>): List<INamedElement> {
        ...
        for (selectedElement in selectedElements) {
            if (selectedElement !is IAssociation
                    || KotlinAssociationConverter(selectedElement).isKotlinCollectionKind(listOf(KotlinAssociationConverter.CollectionKind.MUTABLE_LIST))) {
                continue
            }
    ...
    override fun editElements(elements: List<INamedElement>) {
        // ˅
        elements.forEach {
            KotlinAssociationConverter(it as IAssociation).convertKotlinCollectionKind(listOf(KotlinAssociationConverter.CollectionKind.MUTABLE_LIST))
            if (it.memberEnds[0].navigability == "Navigable") {
                println("Converted to MutableList :".plus(it.memberEnds[0].getFullName(".")))
            } else {
                println("Converted to MutableList :".plus(it.memberEnds[1].getFullName(".")))
            }
        }
        // ˄
    }
